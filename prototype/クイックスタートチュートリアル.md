...
### 「neowise: Not Found」エラーが出る場合

**原因**: データベースファイルが正しい場所にない、またはAPIがデータベースを見つけられていない
...
```bash
# backendディレクトリにファイルがあるか確認
ls -la prototype/backend/neowise_target_region.db
...
```
✅ データベースを発見: /path/to/neowise_target_region.db
```
...
正常なレスポンス例：
```json
{"neowise_count": 10, "neowise_sources": ["4094283750810051968", ...]}
```
...
```bash
# データベース内の天体一覧を確認
sqlite3 neowise_target_region.db "SELECT source_id FROM sources"
```

### ポート8000がすでに使用されている場合
...
ブラウザのコンソールに「Chart is not defined」と表示される場合：

1. Chart.js CDNがブロックされている可能性があります
2. データは正常に取得できているので、グラフが表示されないだけです
3. 天体情報パネルにデータ件数が表示されていれば、API接続は成功しています
