...
容量削減のため、以下の丸め処理を適用しています：

### 生データ（neowise_raw_observations）
| カラム | 丸め | 例 |
|--------|------|-----|
| `mpro` | 小数点以下4桁 | 12.3456789 → 12.3457 |
| `sigmpro` | 小数点以下4桁 | 0.0234567 → 0.0235 |
| `mpro_corrected` | 小数点以下4桁 | 12.3412345 → 12.3412 |

### エポック集約データ（neowise_epoch_summary）
| カラム | 丸め | 例 |
|--------|------|-----|
| `mjd_mean` | 整数 | 59456.7 → 59457 |
| `mag_mean` | 小数点以下4桁 | 12.3456789 → 12.3457 |
| `mag_se` | 小数点以下4桁 | 0.0012345 → 0.0012 |
| `mag_lim` | 小数点以下4桁 | 0.0234567 → 0.0235 |

## Jupyter Notebookでの使用方法

以下のコードをJupyter Notebookで実行してください。
...
import astropy.coordinates as coord
import astropy.units as u
from tqdm.notebook import tqdm  # Jupyter用のtqdm

# zp_stbの読み込み（パスはご自身の環境に合わせてください）
zp_stb = pd.read_csv('/Users/yukikojima/Downloads/NEOWISE_zp_stb.csv', skiprows=12).rename(columns={'scan': 'scan_id'})
...
def save_raw_observations(raw_df, source_id, zp_stb_df, cursor):
    """生の観測データをSQLiteに保存（等級は小数点以下4桁に丸める）"""
...
def process_band_and_save_epochs(table_df, band, source_id, zp_stb_df, cursor):
    ...
    # SQLiteに保存（丸め処理）
    for _, row in result.iterrows():
        mjd_mean_rounded = int(round(row['mjd']))  # 整数に丸め
    ...
    print(f"  {band}: {len(result)} epochs")
    return result
```
...
def get_neowise_and_save(ra, dec, source_id, conn, zp_stb_df):
    ...
    if raw_df.empty or len(set(raw_df['allwise_cntr'])) != 1:
        print(f"No valid data for {source_id}")
        return
    ...
    if raw_df.empty:
        print(f"No data after MJD filtering for {source_id}")
        return
...
for source_id, ra, dec in tqdm(sources):
    try:
        get_neowise_and_save(ra, dec, source_id, conn, zp_stb)
    except Exception as e:
...
print(f"Total sources: {sources_count['count'].iloc[0]}")

# 生データのサンプル（丸め確認）
raw_sample = pd.read_sql_query("""
    SELECT source_id, mjd, band, mpro, sigmpro, mpro_corrected 
    FROM neowise_raw_observations LIMIT 5
""", conn)
print("\n生データサンプル:")
print(raw_sample)

# エポック集約データのサンプル（丸め確認）
epoch_sample = pd.read_sql_query("""
    SELECT source_id, band, mjd_mean, mag_mean, mag_se, mag_lim, n_points 
    FROM neowise_epoch_summary LIMIT 5
""", conn)
print("\nエポック集約データサンプル:")
print(epoch_sample)
