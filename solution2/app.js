    ...
    */
    ...
    async togglePNGView() {
        ...
        if (this.mimizukuPNGMode) {
            ...
            // 案2: 分離モードか通常モードかに応じて復元
            if (this.mimizukuParams.separatedMode) {
                this.mimizukuAladins = null;
    ...
    async convertToPNGView() {
        ...
        try {
            ...
            // Release Aladin instance(s)
            this.mimizukuAladin = null;
            this.mimizukuAladins = null;

            const container = document.getElementById('mimizuku-view-container');
            const modeLabel = this.mimizukuParams.separatedMode ? '案2: 分離表示モード' : '通常モード';
            const sepInfo = this.mimizukuParams.separatedMode ? 
                `(天体間距離: ${(this.mimizukuParams.separation * 60).toFixed(1)}', 閾値: 10' 以上)` : '';
            ...
            if (container) {
                ...
                <div style="display:flex;flex-direction:column;align-items:center;gap:10px;width:100%;height:100%;padding:10px;box-sizing:border-box;">
                  <div style="color:#fff;font-size:18px;font-weight:bold;">MIMIZUKU Dual Field (2'×2') - ${modeLabel}</div>
                  ...
                  <div style="color:#aaa;font-size:14px;">Left: ${this.selectedStar.ra > this.targetCoord.ra ? 'Guide' : 'Target'} | Right: ${this.selectedStar.ra > this.targetCoord.ra ? 'Target' : 'Guide'} (1'×2' each) ${sepInfo}</div>
                </div>`;
            }
    ...
    async convertToPNGViewSeparated() {
        ...
        if (!targetDiv || !guideDiv) {
            throw new Error('Separated field divs not found');
        }
        ...
        // Target側の切り出し
        const targetGeom = this.getAladinCanvasGeomForPNG_Separated(targetDiv);
        const targetScale = targetCanvas.width / targetGeom.cssWidth;
        const leftCanvas = this.extractOneFieldAsCanvas(
            targetCanvas, this.mimizukuAladins.target, target, 
            wDeg, hDeg, targetGeom, targetScale, angleRad, target, guide
        );
        
        // Guide側の切り出し
        const guideGeom = this.getAladinCanvasGeomForPNG_Separated(guideDiv);
        const guideScale = guideCanvas.width / guideGeom.cssWidth;
        const rightCanvas = this.extractOneFieldAsCanvas(
            guideCanvas, this.mimizukuAladins.guide, guide, 
            wDeg, hDeg, guideGeom, guideScale, angleRad, target, guide
        );
        
        // 2つのキャンバスを結合
        const isGuideRight = guide.ra > target.ra;
        const finalLeft = isGuideRight ? leftCanvas : rightCanvas;
        const finalRight = isGuideRight ? rightCanvas : leftCanvas;
        
        const combo = document.createElement('canvas');
        combo.width = finalLeft.width + finalRight.width;
        combo.height = Math.max(finalLeft.height, finalRight.height);
        ...
        g.lineWidth = 3;
        g.beginPath();
        g.moveTo(finalLeft.width, 0);
        g.lineTo(finalLeft.width, combo.height);
        g.stroke();
    ...
    // 案2: 分離ビュー用のジオメトリ取得
    getAladinCanvasGeomForPNG_Separated(containerDiv) {
        const inner = containerDiv?.querySelector('canvas');
        if (!inner) {
            throw new Error('Canvas not found in separated field');
        }
        const outerRect = containerDiv.getBoundingClientRect();
        const innerRect = inner.getBoundingClientRect();
        return {
            innerCanvas: inner,
            cssOffsetX: innerRect.left - outerRect.left,
    ...
    */
    ...
    showMimizukuDualField() {
        ...
        if (sep > this.SEPARATION_THRESHOLD_DEGREES) {
            ...
            // 通常モード: 従来の単一ビュー
            this.mimizukuParams.separatedMode = false;
            setTimeout(() => this.initMimizukuSingleField(), 100);
        }
    }
    ...
    initMimizukuSeparatedFields() {
        ...
        container.innerHTML = `
            <div style="display: flex; flex: 1; gap: 10px; min-height: 0;">
                <div style="flex: 1; display: flex; flex-direction: column; background: #1a1a1a; border: 2px solid #444; border-radius: 4px;">
                    <div style="padding: 10px; background: #333; border-bottom: 2px solid #444; color: #ff0; font-weight: bold; text-align: center;">Target</div>
                    <div id="mimizuku-field-target" style="flex: 1; background: #000; position: relative; min-height: 0;"></div>
                </div>
                <div style="flex: 1; display: flex; flex-direction: column; background: #1a1a1a; border: 2px solid #444; border-radius: 4px;">
                    <div style="padding: 10px; background: #333; border-bottom: 2px solid #444; color: #0ff; font-weight: bold; text-align: center;">Guide Star</div>
                    <div id="mimizuku-field-guide" style="flex: 1; background: #000; position: relative; min-height: 0;"></div>
                </div>
            </div>
        `;
    ...
    // 案2: 個別のフィールドボックスを描画
    drawMimizukuFieldBox(aladin, obj, label, color) {
        const { angleRad, fovWidthArcmin, fovHeightArcmin } = this.mimizukuParams;
        const halfW = (fovWidthArcmin / 60) / 2;
        const halfH = (fovHeightArcmin / 60) / 2;
        
        const ov = A.graphicOverlay({ color: color, lineWidth: 3 });
        ...
        const fill = label === 'Target' ? 'rgba(255,255,0,0.1)' : 'rgba(0,255,255,0.1)';
        ov.add(A.polygon(corners, { color: color, lineWidth: 3, fillColor: fill }));
        
        const shape = label === 'Target' ? 'square' : 'circle';
        cat.addSources([A.source(obj.ra, obj.dec, { name: label, shape: shape, color: color, size: 20 })]);
    }
    ...
    closeMimizukuDualField() {
        ...
        this.mimizukuAladin = null;
        this.mimizukuAladins = null; // 案2: 複数Aladinインスタンスをクリア
        this.mimizukuPNGMode = false;
        const button = document.getElementById('png-mode-text');
        if (button) button.textContent = 'Switch to PNG View';
        const container = document.getElementById('mimizuku-view-container');
